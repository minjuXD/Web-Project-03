...
<head>
<meta charset="UTF-8">
<title>우리 동네 카페 탐험기 - 토박이 회원 정보 수정 페이지</title>
		...
		<div class="row">
			...
			<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
				<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-5 pb-2 mb-3 border-bottom page">
					<h2><a href="myRecords.jsp" style="color:black;">나의 탐험 정보</a> > 토박이 회원 정보 수정</h2>
				</div>
	<div>
		<%
			String sql="";
			try{
				sql="select userName,gender,birth,email,phone,address,theme,postcode,extraAddr,detailAddr,preferDrk,preferDst,cafeName,businessID from CCE24MEMBER where userID=?";
				...
				if(rs.next()){
					...
					String theme=rs.getString(7);
					String sample6_postcode=rs.getString(8);
					String sample6_extraAddress=rs.getString(9);
					String sample6_detailAddress=rs.getString(10);
					String preferDrk=rs.getString(11);
					String preferDst=rs.getString(12);
					String cafeName=rs.getString(13);
					String businessID=rs.getString(14);
					String em[]=email.split("@");
					String drk[]=preferDrk.split(",");
					String dst[]=preferDst.split(",");
				
					%>
					<form name="form" method="post" action="memberUpdAction.jsp">
					<!-- 멤버 타입 선택 -->
						<div class="btn-group mb-3" role="group">
							<input type="radio" class="btn-check" name="mebType" id="exp" name="mebType" value="exp" disabled>
					...
					<!-- 공통 항목 입력 부분 -->
						<div id="common">
							<div id="comID" class="input-group mb-3" style="width:200px;">
								<input type="text" class="form-control" value="<%=sessionID %>" id="userID" name="userID" readonly>
							</div>
							...
							<div id="comName" class="input-group mb-3" style="width:200px;">
								<input type="text" class="form-control" value="<%=userName %>" id="userName" name="userName" placeholder="이름">
							</div>
							<div id="comGend" class="input-group mb-3" style="width:200px;">
								<input type="radio" class="btn-check" id="gender-T" name="gender" value="M" <%=gender.equals("M") ? "checked":"" %>>
								<label class="btn btn-outline-dark" for="gender-T">남</label>
								<input type="radio" class="btn-check" id="gender-T" name="gender" value="F" <%=gender.equals("F") ? "checked":"" %>>
								<label class="btn btn-outline-dark" for="gender-F">여</label>
							</div>
							<div id="comBt" class="input-group mb-3" style="width:200px;">
								<input type="text" class="form-control" value="<%=birth %>" id="birth" name="birth" placeholder="생년월일 8자리">
							</div>
							<div id="comMail" class="input-group mb-3" style="width:600px;">
								<input type="text" class="form-control me-2"  value="<%=em[0] %>" name="mailFront" placeholder="이메일"> @
								<input type="text" class="form-control me-2 ms-2"  value="<%=em[1] %>" name="mailEnd" id="mailEnd">
							...
							<div id="comPh" class="input-group mb-3" style="width:200px;">
								<input type="text" class="form-control" value="<%=phone %>" id="phone" name="phone" placeholder="휴대전화 번호 11자리">
							</div>
							<div id="comAddr" class="input-group mb-3">
								<input type="text" class="form-control me-2" value="<%=sample6_postcode %>" id="sample6_postcode" name="sample6_postcode" placeholder="우편번호" readonly>
								<input type="button" class="fs-6 p-1 me-2 btn btn-outline-dark" onclick="sample6_execDaumPostcode()" value="우편번호 찾기"><br>
								<input type="text" class="form-control me-2" value="<%=sample6_address %>" id="sample6_address" name="sample6_address" placeholder="주소" readonly>
								<input type="text" class="form-control me-2" value="<%=sample6_detailAddress %>" id="sample6_detailAddress" name="sample6_detailAddress" placeholder="상세주소">
								<input type="text" class="form-control me-2" value="<%=sample6_extraAddress %>" id="sample6_extraAddress" name="sample6_extraAddress" placeholder="참고항목" readonly>
							</div>
						</div>
						...
						<!-- 토박이 특정 입력 부분 -->
						<div id="natBox" class="box mt-4">
							<h5>카페 정보</h5>
							<div id="natCafeName" class="input-group mb-3 ms-4" style="width:200px;">
								<input type="text" class="form-control" value="<%=cafeName %>" id="cafeName" name="cafeName" placeholder="카페명 입력">
							</div>
							<div id="natBsID" class="input-group mb-3 ms-4" style="width:200px;">
								<input type="text" class="form-control" value="<%=businessID %>" id="businessID" name="businessID" placeholder="사업자번호 입력">
							</div>
							<div id="natTheme" class="input-group mb-3 ms-4 ps-2 pt-2 border rounded-3">
								<p>카페 테마 : </p><br>
								<%
									try{
					...
					<!-- 제출 버튼 -->
						<div class="mx-3">
							...
							<input type="button" class="fs-6 p-1 ms-2 btn btn-outline-dark" style="width:70px;" value="회원 탈퇴" onclick="location.href='memberDelete.jsp'">
						</div>
						<%
						}}catch(Exception e){
							System.out.println("테이블 읽기 오류");
